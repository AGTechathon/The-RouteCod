    ...
    if (validateStep()) {
      ...
      try {
        if (method === 'ai') {
          ...
          console.log("before");
          console.log('Recommended Spots:', response); // Log the spots to the console
          navigate('/dashboard/plan/manual/generate', { state: { tripId: response.tripId, tripData, lunch:response.lunch, stay:response.stay, spots:response.spots } });
        }
      } catch (error) {
